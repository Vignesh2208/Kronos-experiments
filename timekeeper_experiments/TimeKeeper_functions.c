...
int progress_exp_cbe(int n_rounds) {
	char command[100];
	memset(command,0,100*sizeof(char));
	if (is_root() && isModuleLoaded() && n_rounds > 0) {
		sprintf(command, "%c,%d", PROGRESS_INTERVAL_CBE, n_rounds);
		if (send_to_timekeeper(command) == -1)
			return -1;
                return 0;
...
int set_cbe_exp_timeslice(long timeslice) {
	char command[100];
	memset(command,0,100*sizeof(char));
	if (is_root() && isModuleLoaded() && timeslice > 0) {
		sprintf(command, "%c,%d", SET_CBE_EXP_TIMESLICE, timeslice);
		if (send_to_timekeeper(command) == -1)
			return -1;
                return 0;
